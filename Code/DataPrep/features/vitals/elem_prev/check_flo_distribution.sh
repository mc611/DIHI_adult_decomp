...
# a temporary output file that stores distinct patient ids for the top 200 (combined from top 100 from entire cohort and top 100 from icu admission patients)
...
# flo_top_names.csv: a csv file that stores the top flo meas names
for file in P:/dihi_qi/data_pipeline/data/vitals/*.csv
do
    echo Now scanning file: $file
    awk -F, \
    'FNR==NR{a[$1]++;next};(FNR+200)==NR{b[$1]++;next};a[$3]&&b[$1]&&(!seen[$1,$3]++){print $3 "," $1}' \
    ../../../../../Data/Processed/features/vitals/flo_top_names.csv \
